...
<body>
  ...
  <div class="container">
  <h1 style="text-align: center; padding: 80px;">Kelola Data Pengumuman</h1>
  <a href="uppeng.php" class="btn btn-success">Tambah</a> <br><br>   
        <table class="table">
            <thead>
              <tr>
            ...
            <tbody>
            <?php
					    $selectQuery = "select * from pengumuman";
					    $squery = mysqli_query($koneksi, $selectQuery);
              $no = 1;
              if(isset($_GET['idpeng'])){
             
                mysqli_query($koneksi, "delete from pengumuman where idpeng='$_GET[idpeng]'");
                echo "Data Terhapus";
                echo '<script language = "Javascript">document.location="datapeng.php";</script>';
                ...
                <td><?php echo $result['isi']; ?></td>
				        <td><a href='?idpeng=<?php echo $result['idpeng']?>' onclick="return confirm('Yakin hapus data?')" class="btn btn-danger">Hapus</a></td>
				        </tr>
