...
<body>
   ...
   <header>
    ...
    <div class="headprestasi" style="background-image: url(https://cdn-cms.pgimgs.com/areainsider/2022/09/Alt-Text-3.-SMA-Terbaik-di-Jakarta-SMAN-81-Jakarta-Timur-738x415.png);height: 500px; width: 100%; background-repeat: no-repeat; background-size: cover;">
        <div class="d-flex justify-content-center align-items-end">
           <img style="height: 200px;" src="" alt="">
        </div>
        <div class="container"><h2 class="text-light" style="text-align: center; font-size: 40px; padding-top: 40px;"><b>Pengumuman</b></h2></div>
    </div>
    <div class="container  text-dark rounded-5" style="box-shadow:  2px 2px 2px rgba(0,0,0,0.8); padding: 2px; margin-top: -150px;">
        <?php
        $jmlPeng = mysqli_num_rows($hasil);
        if ($jmlPeng > 0) {
            while ($row = mysqli_fetch_assoc($hasil)) {
                $tanggal = $row['tanggal'];

                $dd = date('d', strtotime($tanggal));
                $mm = date('F', strtotime($tanggal));
                $yy = date('y', strtotime($tanggal));

        ?>
        <div class="container py-4 p my-4 text-white rounded-4" style="width: 85%; background-color: #e6ebe7;">
        <div class="d-flex">
            <div class="container-sm mx-4 bg-secondary rounded-4" style="width: 25%;">
                <div class="py-5 my-4 event-date text-center">
                    <h1 style="font-size: 280%;"><?= $dd; ?></h1> <span><?= $mm; ?> <?= $yy; ?></span>
                </div>
            </div>
            <div class="ml-3">
                <h2 class="mb-3" style="color: gold;"><?= $row["judul"]; ?></h2>
                <p style="font-size: 20px; color : black;"><?= $row["isi"]; ?></p>
            </div>
        </div>
        </div>
        <?php
