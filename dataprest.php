...
<body>
  ...
  <div class="container">
  <h1 style="text-align: center; padding: 80px;">Kelola Data Prestasi</h1>
  <a href="uploadp.php" class="btn btn-success">Tambah</a> <br><br>   
        <table class="table">
            <thead>
              <tr>
            ...
            <tbody>
            <?php
					    $selectQuery = "select * from prestasi";
					    $squery = mysqli_query($koneksi, $selectQuery);
              $no = 1;
              if(isset($_GET['idprestasi'])){
             
                mysqli_query($koneksi, "delete from prestasi where idprestasi='$_GET[idprestasi]'");
                echo "Data Terhapus";
                echo '<script language = "Javascript">document.location="dataprest.php";</script>';
                ...
                <td><?php echo $result['isi']; ?></td>
				        <td><a href='?idprestasi=<?php echo $result['idprestasi']?>' onclick="return confirm('Yakin hapus data?')" class="btn btn-danger">Hapus</a></td>
				        </tr>
