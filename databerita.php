...
<body>
  ...
  <div class="container">
  <h1 style="text-align: center; padding: 80px;">Kelola Data Berita</h1>
  <a href="upberita.php" class="btn btn-success">Tambah</a> <br><br>   
        <table class="table">
            <thead>
              <tr>
            ...
            <tbody>
            <?php
					    $selectQuery = "select * from berita";
					    $squery = mysqli_query($koneksi, $selectQuery);
              $no = 1;
              if(isset($_GET['idberita'])){
             
                mysqli_query($koneksi, "delete from berita where idberita='$_GET[idberita]'");
                echo "Data Terhapus";
                echo '<script language = "Javascript">document.location="databerita.php";</script>';
                ...
                <td><?php echo $result['isi']; ?></td>
				        <td><a href='?idberita=<?php echo $result['idberita']?>' onclick="return confirm('Yakin hapus data?')" class="btn btn-danger">Hapus</a></td>
				        </tr>
